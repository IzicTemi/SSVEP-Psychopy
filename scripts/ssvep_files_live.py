...
def get_filtered_eeg(eeg, lowcut, highcut, order, sample_rate):
    ...
    trial_len = int(250+0.14*sample_rate+3*sample_rate-1) - int(250+0.14*sample_rate)
    filtered_data = np.zeros((trial_len, eeg.shape[1]))

    for channel in range(0, num_chan):
        signal_to_filter = np.squeeze(eeg[int(250+0.14*sample_rate):
                                        int(250+0.14*sample_rate+3*sample_rate-1), channel])
...
def get_segmented_epochs(data, window_len, shift_len, sample_rate):
    ...
    number_of_segments = int(math.ceil((data.shape[0] - data_overlap)/
                                       (duration - data_overlap)))
    
    segmented_data = np.zeros((data.shape[1], number_of_segments, duration))


    for channel in range(0, num_chan):
        segmented_data[channel, :, :] = buffer(data[:, channel], duration, data_overlap) 
